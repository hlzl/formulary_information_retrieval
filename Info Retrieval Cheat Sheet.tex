...
\item Setting $\boldsymbol{L}$ equal to the SVD ($p$ dimensions with $k$; $rank(\boldsymbol{L})=k$) minimizes the Frobenius norm of the difference between $\boldsymbol{X}$ and $\boldsymbol{L}$ given that the dimension $k$ of the subspace which $\boldsymbol{L}$ lies in is known beforehand. \\
\hspace{5}[ $\arg\min_{rank(\boldsymbol{L})}||\boldsymbol{X}-\boldsymbol{L}||_F$] 
\end{itemize}
...
\item Letting $f$ and $g$ be linear mappings represented by matrices, the reconstruction error $J(\mathbf{W,V})=\sum_{i=1}^n||\boldsymbol{x_i-WVx_i}||^2$ is minimised by setting $f$ as the transpose of $U_k$ and $g$ as $U_k$.
\begin{emphbox}

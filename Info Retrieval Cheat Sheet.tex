...
\end{emphbox}
\begin{itemize}
\item The \emph{empirical covariance matrix} or \emph{Score Matrix} of the reduced $k$ variables $\boldsymbol{S}=\boldsymbol{U}_k^{\top}\boldsymbol{X}$ is diagonal, i.e. the features extracted by the PCA are uncorrelated.        [$cov(\boldsymbol{S})=\frac{1}{n}\boldsymbol{S}\boldsymbol{S}^{\top}=\frac{1}{n}\boldsymbol{U}^{\top}\boldsymbol{X}\boldsymbol{X}^{\top}\boldsymbol{U}$]\\
...
\begin{itemize}
\item The error model interpretation is that the samples $\boldsymbol{X}$ is obtained via the addition between a signal matrix $\boldsymbol{L}$ (\emph{clean data}) and a noise matrix $\boldsymbol{N}$, where $\boldsymbol{L}$ is lying on a lower dimensional subspace than $\boldsymbol{X}$, and the goal is to try and find $\boldsymbol{L}$.
[$\boldsymbol{X}=\boldsymbol{L}+\boldsymbol{N}$]
\item Setting $\boldsymbol{L}$ equal to the SVD ($p$ dimensions with $k$; $rank(\boldsymbol{L})\le k$) minimizes the Frobenius norm of the difference between $\boldsymbol{X}$ and $\boldsymbol{L}$ given that the dimension $k$ of the subspace which $\boldsymbol{L}$ lies in is known beforehand. \\
$ \hat{\boldsymbol{L}}=\arg\min_{rank(\boldsymbol{L})}||\boldsymbol{X}-\boldsymbol{L}||_F=\boldsymbol{U}_k\mathrm{diag}(d_1,..,d_k)\boldsymbol{V}_k^{\top}$
\end{itemize}
...
\item Letting $f$ and $g$ be linear mappings represented by matrices, the reconstruction error $J(\mathbf{W,V})=\sum_{i=1}^n||\boldsymbol{x}_i-\boldsymbol{WVx}_i||^2$ is minimised by setting $f$ as the transpose of $U_k$ and $g$ as $U_k$.
\begin{emphbox}
...
\begin{itemize}
    ...
    \hspace{5}[ $c>0$\emph{ and weighs between large margin and misclassification }\\$\rightarrow$\emph{ larger }$c$\emph{, the more the violation of the separation rule is punished.}]
\end{itemize}
\end{sectionbox}
...
\begin{itemize}
    ...
    \emph{Triangular Irregularity}: $d(x,z)\ge d(x,y)+d(x,z)$
...
\begin{itemize}
    ...
    \item \textbf{Precision}: $\frac{\text{# correctly returned relevant answers}}{\text{total items returned by ranker}}$
    \item \textbf{Recall}: $\frac{\text{# correctly returned relevant answers}}{\text{total relevant items}}$
    \item \textbf{F1-Score}: $F_1= 2*\frac{precision*recall}{precision+recall}$
\end{itemize}
